...
public class Test
{
    ...
    [Fact]
    public void TestBytecode() => Directory.GetParent(Environment.CurrentDirectory)!.Parent!.Parent!.GetDirectories("examples").First().GetFiles().Select(v => XElement.Load(v.OpenRead())).ToList().ForEach(v => Assert.Equal(Convert.FromBase64String(v.attr("bytecode") ?? "").ToHexString(), v.finalize().ToHexString()));
}
